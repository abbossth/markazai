...
export function DashboardMock({ host }: { host: string }) {
  ...
  return (
    <div className="bg-ink shadow-brand-900/40 mx-auto w-full max-w-3xl overflow-hidden rounded-2xl border border-white/10 shadow-2xl ring-1 ring-black/5">
      ...
      <div className="flex text-xs">
        <div className="hidden w-36 shrink-0 flex-col gap-1 border-r border-white/10 p-3 sm:flex">
          {["Bosh sahifa", "Lidlar", "Guruhlar", "Talabalar", "Moliya", "Hisobotlar"].map((item, i) => (
            <div key={item} className={`rounded-md px-2.5 py-1.5 ${i === 0 ? "bg-brand-500/20 text-brand-300" : "text-slate-400"}`}>
              {item}
            </div>
          ))}
        </div>
        <div className="flex-1 p-4">
          ...
          <div className="flex h-20 items-end gap-1.5 rounded-lg border border-white/10 bg-white/5 p-3">
            {bars.map((h, i) => (
              <div key={i} className="from-brand-500 to-brand-300 flex-1 rounded-t-sm bg-gradient-to-t" style={{ height: `${h}%` }} />
            ))}
          </div>
        </div>
      </div>
